...
private:
    ...
    struct LiveSegment {//活跃段
        ...
        bool operator&(const struct LiveSegment &that) const {//有重叠
            return this->inside(that.begin) || 
                   this->inside(that.end - 1 > that.begin ? that.end - 1 : that.begin) ||
                   that.inside(this->begin) || 
    ...
    std::list<LiveSegment> segments{};//段列表
    int reference_count;
...
public:
    ...
    bool operator&(const LiveInterval &that) const {
        ...
        if (segments.empty() || that.segments.empty()){
            ...
            auto u = segments.begin();
            auto v = that.segments.begin();
            while (1) {
                if (*u & *v) {//重叠
                    return true;
                }
                if (u->end <= v->begin) {//v大
                    u++;
                    if (u == segments.end()) {//退出循环
                        break;
                    }
                } else if (v->end <= u->begin) {
                    v++;
                    if (v == that.segments.end()) {
                        break;
                    }
                }
            }
