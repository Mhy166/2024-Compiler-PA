...
void RegisterAllocation::Execute() {
    ...
    while (!not_allocated_funcs.empty()) {
        ...
        numbertoins.clear();
        // 对每条指令进行编号
        InstructionNumber(unit, numbertoins).ExecuteInFunc(current_func);//函数内执行，全部编号！
...
void RegisterAllocation::UpdateIntervalsInCurrentFunc() {
    ...
    while (it->hasNext()) {
        ...
        for (auto reverse_it = mcfg_node->Mblock->ReverseBegin(); reverse_it != mcfg_node->Mblock->ReverseEnd();
             ++reverse_it) {
            auto ins = *reverse_it;
            for (auto reg : ins->GetWriteReg()) {//定义！
    ...
    // for(auto [reg,ints]:intervals){
    //     std::cout<<reg.reg_no<<" "<<reg.is_virtual<<std::endl;
    //     for(auto i:ints){
