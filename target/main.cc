...
int main(int argc, char **argv) {
    ...
    ast_root->TypeCheck();
    if (error_msgs.size() > 0) {//有错误信息就输出，不再往后进行
    ...
    if (optimize_flag) {
        ...
        PostDomAnalysis postdom(&llvmIR);
        postdom.Execute();
        (DCEPass(&llvmIR,&postdom)).Execute();
        FIP(&llvmIR).Execute();//函数内联
        llvmIR.BuildCFG();
    }

    if (strcmp(argv[step_tag], "-llvm") == 0) {//输出中间代码
        llvmIR.printIR(fout);
        fout.close();
        return 0;
    }
