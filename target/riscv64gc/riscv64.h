...
#pragma GCC diagnostic ignored "-Wwritable-strings"//这个指令告诉 GCC 编译器忽略与可写字符串常量相关的警告。
#pragma GCC diagnostic ignored "-Wc99-designator"//这个指令用于忽略与 C99 中的设计ator（指定器）相关的警告。
...
struct RiscVLabel {
    ...
    RiscVLabel(int jmp_id):jmp_label_id(jmp_id){};
};
//指令
...
//继承机器的3个数据结构
    ...
    public:
        ...
        std::vector<RiscV64Instruction *> use_stack_ins;
        void UpdatePara_In_UseStack(){
            for(auto ins:use_stack_ins){
                ins->setImm(ins->getImm()+para_sz);
            }
        }
    private:
