...
//一个模板函数，用于打印汇编指令，具体根据指令类型调用不同的打印方法
//通过操作码从 OpTable 查找指令名称。
//根据指令的格式类型（如 R_type、I_type）打印相应的寄存器或立即数。
//支持不同格式类型的指令（例如 R 类型、I 类型、S 类型等）。
...
template <> void RiscV64Printer::printAsm<RiscV64Instruction *>(RiscV64Instruction *ins) {
    s << OpTable[ins->getOpcode()].name << "\t\t";//名字
...
//打印 PHI 指令（用于基本块之间的合并）
//打印合并结果及每个标签和对应操作数的映射
...
//基本指令打印函数
//根据指令的架构类型，调用适当的 printAsm 函数打印指令
//如果指令是 RISC-V 64 类型，调用 printAsm<RiscV64Instruction *>
//如果是 PHI 类型，调用 printAsm<MachinePhiInstruction *>。
template <> void RiscV64Printer::printAsm<MachineBaseInstruction *>(MachineBaseInstruction *ins) {
...
//里面调用printasm
//emit 是整个打印过程的主函数。首先输出一些全局设置，如 .text、.globl main 和架构信息
//然后遍历所有函数和块，打印每个指令，调用相应的 printAsm 函数
//也会打印全局变量和字符串常量定义
    ...
    for (auto global : printee->global_def) {
        if (global->GetOpcode() == BasicInstruction::GLOBAL_VAR) {
            ...
            if (global_ins->type == BasicInstruction::I32) {
                if (global_ins->arrayval.dims.empty()) {//非数组
                    if (global_ins->init_val != nullptr) {//赋初值
