...
//LLVM类型输出 32位整数，32位浮点数，指针，空，8位整数（字符），1位整数（bool），64位整数，双精度。
...
//LLVM指令类型输出 load，store，add，sub，icmp，phi（SSA控制流合并），alloca，mul，div，条件跳转，无条件跳转，位异或，左移等等。
...
//LLVM浮点数比较，比如OEQ，有序且相等，UEQ，无序或相等
