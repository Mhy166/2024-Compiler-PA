...
class BasicOperand {
    ...
    virtual std::string GetFullName() = 0;//该虚函数在Instruction out文件里写了
...
class LabelOperand : public BasicOperand {
    ...
    virtual Operand CopyOperand();
    void SetLabelNo(int no){label_no=no;}
...
class BasicInstruction {
    ...
    virtual void Reg_Rename(std::map<int,int> mem2reg_map)=0;
    virtual void Label_Rename(std::map<int,int> label_map)=0;
};
...
class LoadInstruction : public BasicInstruction {
    ...
    void Reg_Rename(std::map<int,int> mem2reg_map);
    void Label_Rename(std::map<int,int> label_map){};
...
class FcmpInstruction : public BasicInstruction {
    ...
    void SetOp1(Operand op) { op1 = op; }
    void SetOp2(Operand op) { op2 = op; }
    void SetCond(FcmpCond newcond) { cond = newcond; }
    void SetResultReg(Operand op) { result = op; }
...
class PhiInstruction : public BasicInstruction {
    ...
    enum LLVMType type;
    Operand result;
    std::vector<std::pair<Operand, Operand>> phi_list;//label,val
    ...
    virtual void PrintIR(std::ostream &s);//先2后1
    void Reg_Rename(std::map<int,int> mem2reg_map);
    Operand GetResult() { return result; }
    ...
    size_t GetPhiCount() const {return phi_list.size();}
    void Label_Rename(std::map<int,int> label_map);
};
...
class BrUncondInstruction : public BasicInstruction {
    ...
    Operand GetResultReg() { return nullptr; }
    int GetResultRegNo() { return -1; }
    void SetDestLabel(Operand op){ destLabel=op; }
    std::vector<Operand> GetNonResultOperands() { return std::vector<Operand>{}; }
...
class RetInstruction : public BasicInstruction {
    ...
    void SetRetVal(Operand op) { ret_val = op; }
    virtual void PrintIR(std::ostream &s);
    void Reg_Rename(std::map<int,int> mem2reg_map);
    void Label_Rename(std::map<int,int> label_map){};
};
...
typedef FunctionDefineInstruction *FuncDefInstruction;//函数定义
...
class FptosiInstruction : public BasicInstruction {
    ...
    void SetSrc(Operand op) { value = op; }
    void PrintIR(std::ostream &s);
    void Reg_Rename(std::map<int,int> mem2reg_map);
    void Label_Rename(std::map<int,int> label_map){};
};
