...
%}
%union{//定义所有可能的语法符号（tokens 和 nonterminals）的值的联合体。每个成员对应一种可能的类型
    char* error_msg;
    Symbol symbol_token;
...
%type <expressions> Exp_List Array_List_Exp Array_List_Const;
%type <stmt> Stmt 
%type <block> Block
...
%precedence THEN
%precedence ELSE//后声明的符号具有更高的优先级。
%%
...
|IDENT Array_List_Const{
    $$ = new VarDef_no_init($1,$2); 
    $$->SetLineNumber(line_number);
}
|IDENT Array_List_Const '=' VarInitVal{
    $$ = new VarDef($1,$2,$4); 
    $$->SetLineNumber(line_number);
}
;   
...
|IDENT Array_List_Const '=' ConstInitVal{
    $$ = new ConstDef($1,$2,$4); 
    $$->SetLineNumber(line_number);
}
;
...
|INT IDENT '[' ']'{
    ...
    $$ = new __FuncFParam(Type::INT,$2,null);
    $$->SetLineNumber(line_number);
}
|FLOAT IDENT '[' ']'{
    std::vector<Expression>* null = new std::vector<Expression>;
    null->push_back(nullptr);//表示有个null，和没有要区分开来
    $$ = new __FuncFParam(Type::FLOAT,$2,null);
    $$->SetLineNumber(line_number);
}
|INT IDENT '[' ']' Array_List_Const{
    $5->insert($5->begin(),nullptr);
    $$ = new __FuncFParam(Type::INT,$2,$5);
    $$->SetLineNumber(line_number);
}
|FLOAT IDENT '[' ']' Array_List_Const{
    $5->insert($5->begin(),nullptr);
